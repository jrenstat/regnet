		...
		if(robust){
			for(unsigned int j=0; j<lamb2.n_elem ; j++){
				for(unsigned int i=0; i<lamb1.n_elem ; i++){
					fit = RunSurv_robust(xc, xg, y, lamb1(i), lamb2(j), bc, bg, r, a, p, pc, method, debugging, maxit, tol);
			...
			for(unsigned int j=0; j<lamb2.n_elem ; j++){
				for(unsigned int i=0; i<lamb1.n_elem ; i++){
					fit = RunSurv(xc, xg, y, lamb1(i), lamb2(j), bc, bg, r, a, triRowAbsSums, p, pc, method, maxit, tol);
					btmp = Rcpp::as<arma::vec>(fit["b"]);
