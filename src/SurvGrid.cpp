...
// [[Rcpp::export()]]
arma::mat SurvGrid(arma::mat const &xc, arma::mat const &xg, arma::vec const &y, arma::mat const &x2, arma::vec const &y2, arma::vec const &lamb1, arma::vec const &lamb2, arma::vec bc, arma::vec bg, double r, arma::mat const &a, int p, int pc, bool robust, char method, bool debugging, int maxit, double tol)
{
		...
		if(robust){
			for(unsigned int j=0; j<lamb2.n_elem ; j++){
				for(unsigned int i=0; i<lamb1.n_elem ; i++){
					btmp = RunSurv_robust_fit(xc, xg, y, lamb1(i), lamb2(j), bc, bg, r, a, p, pc, method, debugging, maxit, tol, converged, niter, diff);
			...
			for(unsigned int j=0; j<lamb2.n_elem ; j++){
				for(unsigned int i=0; i<lamb1.n_elem ; i++){
					btmp = RunSurv_fit(xc, xg, y, lamb1(i), lamb2(j), bc, bg, r, a, triRowAbsSums, p, pc, method, maxit, tol, converged, niter, diff);
